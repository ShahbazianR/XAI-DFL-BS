    ...
    for episode in tqdm(list(range(RL_episodes)[episode_start:]), desc="\nDFRL Episodes"):
        ...
        while global_timesteps < train_time_steps:
            ...
            for bs in bs_keys:
                ...
                if explain:
                    traffic_volume, attws = Current_BS.TP_model(t, explain=explain)
                    attws = attws.detach().numpy()
                    # print("attw", attws)
                else:
                ...
                if  rand_n < Current_BS.DRL_model.Q_eval.exploration_proba:
                    ...
                    if explain:
                        predicted_mode_actor, actor_probs = Current_BS.DRL_model.choose_action(current_state, explain)  ## the output of the model
                        Current_BS.explore_update()
                        predicted_mode_explore, explore_probs = Current_BS.Explore_model.choose_action(current_state, explain)  ## the output of the model
                    else:
                ...
                if explain:
                    ts_result_dict['Action Taker'] = action_taker
                    ts_result_dict['Probs_0'] = probs.detach().numpy()[0]
                    ts_result_dict['Probs_1'] = probs.detach().numpy()[1]
                    ts_result_dict['Prob_Diff'] = abs(probs.detach().numpy()[1])-abs(probs.detach().numpy()[0]), ## Q(active) - Q(sleep)
                    ts_result_dict['W11(avg1_short)'] = attws[0][0]
                    ts_result_dict['W12(max1_short)'] = attws[0][1]
                    ts_result_dict['W21(avg2_long)'] = attws[1][0]
                    ts_result_dict['W22(max2_long)'] = attws[1][1]
